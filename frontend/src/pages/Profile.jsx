...
export default function Profile() {
    ...
    const impact = {
        submitted: dashboard?.submitted ?? 0,
        verified: dashboard?.verified ?? 0,
        connected: dashboard?.connected ?? 0,
    };

    // Privacy state (persisted to localStorage)
    const [privacySettings, setPrivacySettings] = useState(() => {
        try {
            const saved = localStorage.getItem("nirikshan_privacy_settings");
            return saved ? JSON.parse(saved) : { anonymousMode: false, hideLocationData: false, shareWithNGOs: true };
        } catch { return { anonymousMode: false, hideLocationData: false, shareWithNGOs: true }; }
    });

    useEffect(() => {
        localStorage.setItem("nirikshan_privacy_settings", JSON.stringify(privacySettings));
    }, [privacySettings]);
    ...
    const handleSaveInfo = async (e) => {
        ...
        try {
            await api.updateProfile({
                ...
                city_district: userProfile.city,
    ...
    const handleSaveSettings = async (e) => {
        ...
        try {
            await api.post("/api/notifications/preferences", {
                preferences: [
                    { type: "email_alerts", enabled: settings.emailAlerts },
                    { type: "sms_alerts", enabled: settings.smsAlerts },
                    { type: "push_notifications", enabled: settings.pushNotifications },
                ],
            });
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="text-sm text-secondary mt-0.5">
                        {userProfile.city} · Member since {profile?.joined || "2026"}
                    </div>
                    ...
                    <div className="relative bg-card border border-border sm:rounded-2xl rounded-t-2xl w-full sm:max-w-md p-5 sm:p-6 shadow-2xl space-y-5 sheet-enter max-h-[92dvh] overflow-y-auto sm:mx-4" style={{ paddingBottom: "calc(1.25rem + env(safe-area-inset-bottom))" }}>
                        ...
                        <div className="rounded-xl bg-accent/40 border border-border p-3.5 space-y-2 text-xs">
                            ...
                            <div className="flex items-center justify-between text-muted">
                                ...
                                <span className="text-foreground font-medium">{profile?.mobile || "N/A"}</span>
                            </div>
                            <div className="flex items-center justify-between text-muted">
                                <span>Location:</span>
