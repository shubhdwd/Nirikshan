...
function Success({ caseCode, caseId }) {
    return (
...
export default function Report() {
    ...
    const submit = async () => {
        ...
        try {
            ...
            // PCRN (community responder + human verification) is always required so
            // the backend z.array(...).min(1) validation passes and the case can be routed.
            const assistanceTypes = ["PCRN"];
            const emergencyLevel =
                form.emergency === "yes" ? "IMMEDIATE" : "STANDARD";

            const childDescription = [
                obsText,
                `${form.childCount || "?"} child, age ${form.ageBand || "unknown"}`,
                form.stillPresent ? `Present: ${form.stillPresent}` : "",
                form.notes || "",
            ]
                .filter(Boolean)
                .join(". ");
            ...
            const body = {
                child_description: childDescription.slice(0, 5000),
                latitude: form.latitude || 19.076,
                longitude: form.longitude || 72.8777,
                location_label: form.area || form.location || "Unspecified",
                emergency_level: emergencyLevel,
                assistance_types: assistanceTypes,
                assistance_notes: form.notes || "",
            };

            const result = await api.cases.create(body);

            if (form.photoFile && result.id) {
                try {
                    await api.upload(`/api/cases/${result.id}/evidence`, form.photoFile);
                } catch {
                    toast.warning("Report submitted but photo upload failed. You can add it later.");
                }
            }

            setSubmitted({ caseCode: result.case_code, caseId: result.id });
        } catch (err) {
    ...
    const searchLocationQuery = async (query) => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                set({
                    ...
                    area: first.display_name,
                    coordsLocked: true,
                });
                toast.success(`Location found: ${first.display_name.split(',')[0]}`);
            } else {
                toast.error("Location not found. Try a broader search term.");
            }
        } catch {
    ...
    if (submitted && submitted.caseCode) {
        return (
            <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-10 py-6 lg:py-10">
                <Success caseCode={submitted.caseCode} caseId={submitted.caseId} />
            </div>
        );
    }
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-10 py-4 sm:py-6 lg:py-10 space-y-5 sm:space-y-6">
            ...
            {step === 1 && (
                <StepCard
                    ...
                    description="Approximate area is enough. Click on the map or search an area."
                >
                    <label className="block">
                        ...
                        <div className="mt-1.5 flex gap-2">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    onKeyDown={(e) => {
                                        if (e.key === "Enter") {
                                            e.preventDefault();
                                            searchLocationQuery(form.location);
                                        }
                                    }}
                            ...
                            <button
                                type="button"
                                onClick={() => searchLocationQuery(form.location)}
                                className="px-4 py-2.5 bg-primary text-primary-foreground rounded-xl text-xs font-medium hover:bg-primary-hover transition-colors"
                            >
                                Search
                            </button>
                        </div>
                        ...
                        onClick={() => {
                            if (navigator.geolocation) {
                                navigator.geolocation.getCurrentPosition(
                                    ...
                                    () => toast.error("Unable to capture current location. Click on the map to set location.")
                                );
                            }
                        }}
            ...
            <div className="sticky bottom-0 z-20 -mx-4 sm:mx-0 px-4 sm:px-0 py-3 sm:py-0 sm:static bg-background/95 sm:bg-transparent backdrop-blur sm:backdrop-blur-none border-t border-border sm:border-0 flex items-center justify-between gap-3 pt-3">
                ...
                {step < STEPS.length ? (
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-2 rounded-full bg-primary hover:bg-primary-hover text-primary-foreground px-6 py-2.5 text-sm font-medium active:scale-[0.97] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
