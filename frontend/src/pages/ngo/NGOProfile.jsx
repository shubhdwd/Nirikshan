...
export default function NGOProfile() {
    ...
    const [dashboard, setDashboard] = useState({ newReferrals: 0, active: 0, completedThisMonth: 0, completedAllTime: 0, avgResponse: "\u2014" });
    const [org, setOrg] = useState({ name: "", contact: "", initials: "", joined: "", areasServed: [], members: [] });
    ...
    useEffect(() => {
        ...
        api.get("/api/organizations").then((data) => {
            if (Array.isArray(data) && data.length > 0) {
                const o = data[0];
                setOrg(o);
                setSettings((s) => ({ ...s, referralEmail: o.contact || "" }));
            }
        }).catch(() => {});
        api.get("/api/ngo/areas").then(setAreas).catch(() => {});
    }, []);

    const orgName = profile?.name || org.name || "Organization";
    const initials = org.initials || orgName.split(" ").map((s) => s[0]).join("").slice(0, 2).toUpperCase();
    const members = org.members || [];
    const areasServed = areas.length > 0 ? areas : org.areasServed || [];
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-10 py-6 lg:py-10 space-y-10">
            ...
            <section className="rounded-2xl border border-border bg-card p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5">
                ...
                <div className="flex-1 min-w-0">
                    <h2 className="font-display text-2xl font-semibold text-foreground">{orgName}</h2>
                    <div className="text-sm text-secondary mt-0.5">Member since {org.joined || "\u2014"}</div>
                ...
                <div className="rounded-2xl border border-border bg-card p-5 space-y-3 text-sm">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-muted">{settings.referralEmail || "\u2014"}</div>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section>
                ...
                <ul className="rounded-2xl border border-border bg-card divide-y divide-border">
                    {members.length > 0 ? members.map((m, i) => (
                        <li key={m.name || i} className="flex items-center gap-4 p-4">
                    ...
                    )) : <li className="p-6 text-center text-sm text-muted">No members loaded.</li>}
                </ul>
            </section>
            ...
            <section>
                ...
                <div className="rounded-2xl border border-border bg-card p-5 flex flex-wrap gap-2">
                    {areasServed.map((a) => {
                        ...
                        return (<ToneBadge key={label} tone="info"><MapPin className="h-3 w-3" /> {label}</ToneBadge>);
                    })}
                </div>
            </section>
            ...
            <section>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <StatCard label="Active cases" value={dashboard.active} icon={Handshake} tone="info" />
                    <StatCard label="Completed (all time)" value={dashboard.completedAllTime} icon={CheckCircle2} tone="verified" />
                    <StatCard label="Avg response" value={dashboard.avgResponse} icon={Clock3} tone="resolved" />
                </div>
            </section>
            ...
            <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="rounded-2xl border border-border bg-card p-5">
                        <ToneBadge tone="verified">Community Impact Award</ToneBadge>
                        <div className="text-xs text-muted mt-3">Recognized for field coordination across Thane district.</div>
                    </div>
                    <div className="rounded-2xl border border-border bg-card p-5">
                        <ToneBadge tone="info">100+ Families Reached</ToneBadge>
                        <div className="text-xs text-muted mt-3">Sustained verified case response over 12 months.</div>
                    </div>
                    <div className="rounded-2xl border border-border bg-card p-5">
                        <ToneBadge tone="primary">Rapid Response Partner</ToneBadge>
                        <div className="text-xs text-muted mt-3">Avg response under 48 hours across all accepted referrals.</div>
                    </div>
                </div>
            </section>
            ...
            <section>
                ...
                <div className="rounded-2xl border border-border bg-card p-5 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                        <div className="text-sm font-medium text-foreground">{members.length} authorized members</div>
                        <div className="text-xs text-muted mt-0.5">Coverage across {areasServed.length} service areas</div>
                    </div>
