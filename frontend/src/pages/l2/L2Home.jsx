...
export default function L2Home() {
    ...
    const activeCases = (escalations || []).filter((e) => e.status !== "RESOLVED").slice(0, 3);
    const needsAttention = (escalations || []).filter((e) => e.status === "OPEN").length;
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <h2 className="font-display text-xl sm:text-2xl font-semibold text-foreground mt-1">
                            {needsAttention} case{needsAttention === 1 ? "" : "s"} awaiting
                            your update
                        </h2>
                            ...
                            >
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="text-sm font-medium text-foreground truncate">
                                        {c.reason || c.status} · Case {c.case_id}
                                    </div>
                                    <div className="text-xs text-muted mt-0.5">
                                        {c.status} · Escalated
                                    </div>
                                </div>
                                <ToneBadge tone={PRIORITY_TONE[c.status] || "info"}>
