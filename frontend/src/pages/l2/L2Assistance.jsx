...
function IncomingRequestCard({ request, onAccept, onDecline }) {
    const isEmergency = request.emergency_level === "IMMEDIATE" || request.emergency_level === "URGENT";
            ...
            <div className="flex items-start justify-between gap-3">
                <div>
                    ...
                    <div className="text-xs text-secondary mt-0.5">
                        Case {request.case_id} · Escalated by L1
                    </div>
                </div>
                <ToneBadge tone={PRIORITY_TONE[request.emergency_level] || "info"}>
                    {request.emergency_level?.toLowerCase() || "standard"}
                </ToneBadge>
            </div>

            <div className="rounded-xl border border-border bg-background p-3 text-xs text-secondary">
                {request.reason || "Level 2 support requested for this case."}
            </div>
    ...
    return (
        ...
        >
            <div className="flex items-start justify-between gap-3">
                <div>
                    ...
                    <div className="text-sm text-secondary mt-1">
                        {item.status} · Escalation {item.id}
                    </div>
                </div>
                <div className="flex flex-col items-end gap-2">
                    <ToneBadge tone={PRIORITY_TONE[item.emergency_level] || "info"}>
                        {item.emergency_level?.toLowerCase() || "standard"}
                    </ToneBadge>
                </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-background p-4 text-sm text-secondary">
                {item.reason || "Escalation case."}
            </div>

            {/* Actions */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                <ActionBtn
                    onClick={() => toast.success("Update Situation logged.")}
                ...
                <ActionBtn
                    onClick={() => {
                        toast.info(`Requesting Level 3 professional for ${item.id}...`);
                        onProfessional(item);
                    }}
...
export default function L2Assistance() {
    ...
    useEffect(() => {
        Promise.all([
            api.escalations.queue().catch(() => []),
            api.escalations.mine().catch(() => []),
        ]).then(([queueData, mineData]) => {
            setQueue(queueData || []);
            setEscalations(mineData || []);
            setCases((mineData || []).filter((e) => e.status !== "RESOLVED"));
        }).finally(() => setLoading(false));
    }, []);

    const handleAcceptRequest = (r) => {
        const isEmergency = r.emergency_level === "IMMEDIATE";
        if (isEmergency) {
    ...
    const executeAccept = (r) => {
        api.post(`/api/escalations/${r.id}/claim`, {}).then(() => {
            setQueue((prev) => prev.filter((x) => x.id !== r.id));
            setEscalations((prev) => [{ ...r, status: "IN_PROGRESS" }, ...prev]);
            setCases((prev) => [{ ...r, status: "IN_PROGRESS", current: "l2" }, ...prev]);
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10 py-6 lg:py-10 space-y-8">
            ...
            {queue.length > 0 && (
                <section className="space-y-4">
                    <SectionHeader
                        eyebrow="Incoming"
                        title={`Escalation Requests (${queue.length})`}
                    />
            ...
            <section className="space-y-4">
                ...
                />
                {loading ? (
                    <div className="text-center py-8 text-sm text-muted">Loading escalations...</div>
                ) : cases.length ? (
                    <div className="grid grid-cols-1 gap-4">
                        {cases.map((item) => (
                            <CaseRow
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <h3 className="font-display text-xl font-semibold text-foreground">
                                    {emergencyPending.reason || "Emergency Escalation"}
                                </h3>
                                <p className="text-xs text-muted mt-0.5">
                                    {emergencyPending.id} · Case {emergencyPending.case_id}
                                </p>
                            </div>
                        </div>

                        <div className="rounded-xl border border-emergency/30 bg-emergency/10 p-4 space-y-2 text-xs">
                            <p className="font-semibold text-emergency text-sm">Immediate Response Required</p>
                            ...
                            <p className="font-medium text-foreground text-sm pt-1">Can you go to the location right now?</p>
                        </div>

                        <div className="flex flex-col sm:flex-row gap-3 pt-2">
                            <button type="button" onClick={confirmEmergencyAccept} className="flex-1 inline-flex items-center justify-center gap-2 rounded-full bg-emergency hover:bg-emergency/90 text-white px-5 py-2.5 text-sm font-medium shadow-md transition-all">
                                <CheckCircle2 className="h-4 w-4" /> Yes, I can go right now
                            </button>
                            <button type="button" onClick={declineEmergencyAccept} className="flex-1 inline-flex items-center justify-center gap-2 rounded-full border border-border bg-card hover:bg-accent text-foreground px-5 py-2.5 text-sm font-medium transition-colors">
